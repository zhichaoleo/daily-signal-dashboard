...
type WeatherReport = { id: string; district: string; condition: string; temperature: number | null; high: number | null; low: number | null; humidity: number | null; windSpeed: number | null; precipitationProbability: number | null; uvIndex: number | null; forecast: WeatherDay[]; error?: string };
...
type DashboardResponse = { updatedAt: string; weather: WeatherReport[]; stocks: StockReport[]; newsFeed: InsightItem[]; newsSources: NewsSource[]; weiboHot: InsightItem[]; trendingRepos: TrendingRepo[]; notices: string[] };

type OpenMeteoResponse = { current?: { temperature_2m?: number; relative_humidity_2m?: number; weather_code?: number; wind_speed_10m?: number; uv_index?: number }; daily?: { time?: string[]; weather_code?: number[]; temperature_2m_max?: number[]; temperature_2m_min?: number[]; precipitation_probability_max?: number[] } };
...
type RssItem = { title?: string; link?: string; pubDate?: string; description?: string; source?: string | { "#text"?: string }; "media:thumbnail"?: { "@_url"?: string } | Array<{ "@_url"?: string }>; "media:thumbnails"?: { "@_url"?: string } | Array<{ "@_url"?: string }>; "media:content"?: { "@_url"?: string; "@_medium"?: string } | Array<{ "@_url"?: string; "@_medium"?: string }> };
...
export default async (req: Request, _context: Context) => {
  ...
  return json({ updatedAt: new Date().toISOString(), weather, stocks, newsFeed, newsSources: curatedNewsSources, weiboHot, trendingRepos, notices } satisfies DashboardResponse, 200, {
...
async function getWeatherReports(): Promise<WeatherReport[]> {
  const results = await Promise.allSettled(weatherLocations.map(async (location) => {
    const params = new URLSearchParams({ latitude: String(location.latitude), longitude: String(location.longitude), timezone: "Asia/Shanghai", current: "temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m,uv_index", daily: "weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max", forecast_days: "3" });
    ...
    return { id: location.id, district: location.district, condition: weatherCodeText[data.current?.weather_code ?? -1] ?? forecast[0]?.condition ?? "天气更新中", temperature: roundOrNull(data.current?.temperature_2m), high: forecast[0]?.high ?? null, low: forecast[0]?.low ?? null, humidity: roundOrNull(data.current?.relative_humidity_2m), windSpeed: roundOrNull(data.current?.wind_speed_10m), precipitationProbability: forecast[0]?.precipitationProbability ?? null, uvIndex: roundOrNull(data.current?.uv_index), forecast };
  }));
  return results.map((result, index) => result.status === "fulfilled" ? result.value : { id: weatherLocations[index].id, district: weatherLocations[index].district, condition: "暂时不可用", temperature: null, high: null, low: null, humidity: null, windSpeed: null, precipitationProbability: null, uvIndex: null, forecast: [], error: getErrorMessage(result.reason, "Weather request failed") });
}
...
function getRssThumbnail(item: RssItem): string {
  for (const key of ["media:thumbnail", "media:thumbnails", "media:content"] as const) {
    const val = item[key];
    if (Array.isArray(val)) { const u = String(val[0]?.["@_url"] ?? ""); if (u) return u; }
    else if (typeof val === "object" && val) { const u = String(val["@_url"] ?? ""); if (u) return u; }
  }
  return "";
}
